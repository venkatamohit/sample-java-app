...
def review_code(code, repo, pull_number, file_path):
    ...
    response = openai.ChatCompletion.create(
        ...
        messages=[
            ...
            {"role": "user", "content": f"Review the following code for any issues and do a peer code review:\n\n{code_to_review}"}
        ]
    )
...
def post_issue_comments(repo_name, pull_number, file_path, review_result):
    ...
    for issue in issues:
        ...
        if line_number is None:
            ...
            try:
                pull_request = repo_obj.get_pull(pull_number)
                pull_request.create_review_comment(data["body"], data["commit_id"], file_path, data["position"])
